...
class AddWholesalerView extends StatefulWidget {
  const AddWholesalerView({super.key});

  @override
  State<AddWholesalerView> createState() => _AddWholesalerViewState();
}

class _AddWholesalerViewState extends State<AddWholesalerView> {
  // ── Controllers ──────────────────────────────────────────────────────────
    ...
    try {
      ...
      if (_profileImage != null) {
        final request = http.MultipartRequest(
          'POST',
          Uri.parse('${BackendConfigs.apiUrl}wholesaler/add'),
        );
      ...
      if (mounted && newId != null) {
        ...
        final currentList = List<Wholesaler>.from(
          userProvider.getSalesUserDetails()?.wholesalers ?? [],
        );
        currentList.add(newWholesaler);
        userProvider.updateWholesalers(currentList);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: customAppBar(context, text: 'Add Wholesaler', showText: true),
